...
class DbHelper {
  ...
  Future<int> saveData(UserModal user) async {
    var dbClient = await db;
    var res = await dbClient.insert(Table_User, user.toMap());
    return res;
  }

  Future<UserModal?> getLoginUser(String userId, String password) async {
    var dbClient = await db;
    ...
    if (res.length > 0) {
      return UserModal.fromMap(res.first);
    }

    return null;
  }

  Future<int> updateUser(UserModal user) async {
    var dbClient = await db;
